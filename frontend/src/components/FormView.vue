<template>
	<div class="relative bg-white">
		...
		/>
		<!-- <pre>{{ formIsReady }}</pre> -->
		<template v-if="(isExistingDoc || isImageAttachStepComplete) && formIsReady">
				...
				<template #afterForm>
					...
					<div class="sticky bottom-0 w-full bg-white pt-3 pb-5">
						<Button
							...
							appearance="primary"
							:loading="DocTypeList.insert.loading || documentResource?.setValue.loading"
						>
		...
		<!-- Loading Indicator -->
		<div
			v-else-if="!((isExistingDoc || isImageAttachStepComplete) && formIsReady) && isExistingDoc"
...
async function handleUpdate() {
	if (document) {
		...
		for (const file of attachedFiles) {
			if (!file.uploaded) {
				await uploadFileAttachment(props.doctype, props.id, file.model, file.value)
				file.uploaded = true
			}
		}
