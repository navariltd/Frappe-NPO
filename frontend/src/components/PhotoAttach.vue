<template>
	<div class="mb-1 space-y-4 px-4 py-3">
		...
		<div class="grid w-full grid-cols-5 items-center justify-between object-center">
			<div v-for="(image, index) in images" :key="image.filename">
				...
				<a target="_blank" :href="image.url">
					<img class="h-16 w-16 rounded-lg object-cover shadow-md" :src="image.url" />
				</a>
			</div>
		...
		<Button v-if="!props.previewMode" @click="handleComplete" appearance="primary">{{
...
let attachedImages = createResource({
	url: "frappe_npo.api.get_attached_images",
	params: {
