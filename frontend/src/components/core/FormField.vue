<template>
	...
	<div v-bind="$attrs" v-else-if="props.type === 'geolocation'">
		<div v-if="props.modelValue">
			...
			>
				<div>
					<div class="flex w-fit items-center gap-2 rounded-lg bg-black py-2 px-5 text-white">
	...
	<div v-else-if="props.type === 'attach'">
		...
		<div class="flex space-x-2" v-if="props.modelValue && typeof props.modelValue === 'string'">
			...
			<Button class="rounded-lg text-lg" @click="handleAttachmentRemove" appearance="danger"
				>Remove</Button
			>
		</div>
	...
	<div v-else-if="props.type === 'table'">
		...
		<Button class="mb-3 rounded-lg text-lg" icon-left="plus" @click="handleChildAdd($event)"
			>Add Row
		</Button>
		<div class="rounded-xl border border-gray-300">
			<div v-if="!props.modelValue || props.modelValue.length === 0">
				<div class="flex h-32 items-center justify-center text-lg text-gray-500">
			...
			<div v-else>
				<div
					...
					:class="[
						...
						'items-center',
						'border-b',
						index === props.modelValue.length - 1 ? 'border-transparent pb-3' : 'border-gray-300',
						index === 0 ? 'pt-3' : '',
					]"
				>
	...
	<div v-if="props.type === 'table' && toggleChildDetail">
		<div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 px-4">
			<div class="z-10 w-full rounded-xl bg-white px-4 pt-2 pb-4">
				...
				<Button class="mt-3 w-full rounded-lg text-lg" @click="updateChild" appearance="primary"
					>Save</Button
				>
			</div>
		</div>
	</div>
	<ErrorMessage v-if="validation?.meta?.touched" :message="validation?.errorMessage" />
</template>

<script lang="ts" setup>
import FormField from "@/components/core/FormField.vue"
import { sessionInjectionKey } from "@/typing/InjectionKeys"
import { Autocomplete, Button, ErrorMessage, FeatherIcon, createResource } from "frappe-ui"
...
function getFormattedGeolocation(geoCoordinates: { latitude: number; longitude: number }): string {
...
onMounted(() => {
	if (props.type === "link" && props.doctype) {
		...
		doctypeList.value = createResource({
			url: "frappe_npo.api.get_doctype_options",
			params: {
	...
	if (props.type === "table") {
		...
		childTableFields = createResource({
			url: "frappe_npo.api.get_form_fields",
			params: {
				doctype: props.doctype,
			},
			auto: true,
			transform(fields: Array<DocField>) {
				...
				return fields.map((field) => ({
					...
					options: (field.fieldtype != "Link" && field.options && field.options.split("\n")) || [],
				}))
			},
		})
	}
