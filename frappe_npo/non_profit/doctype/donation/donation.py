...
from frappe_npo.non_profit.doctype.membership.membership import verify_signature
...
class Donation(Document):
    ...
    def compute_total_and_validate(self):
        ...
        enable_payment_table = non_profit_settings.get(
            "enable_payment_table_on_donation", 0
        )
        total_amount_paid = calculate_donation_paid_amount(self)

        frappe.db.set_value(
            "Donation", self.name, "total_amount_paid", total_amount_paid
        )
            ...
            for row in donation_payments:
                if flt(self.amount):
                    frappe.db.set_value(
                        ...
                        row.name,
                        "percentage",
                        (flt(row.amount) / flt(self.amount)) * 100,
                    )
                else:
    ...
    def create_payment_entry(self, date=None):
        ...
        from frappe_npo.non_profit.custom_doctype.payment_entry import (
...
def calculate_donation_paid_amount(doc):
    ...
    refs = frappe.get_all(
        ...
        filters={"reference_doctype": "Donation", "reference_name": doc.name},
